...
exports.addQuestionComment = catchAsync(async (req, res, next) => {
  ...
  const comment = await Comment.create({
    ...
    questionId: req.params.questionId,
    content:
      !req.body.content && req.file ? req.file.filename : req.body.content,
...
exports.deleteQuestionComment = catchAsync(async (req, res, next) => {
  ...
  try {
    ...
    return next(
      new AppError("Error deleting comment and associated data", 500)
    );
  }
});
...
exports.addPostComment = catchAsync(async (req, res, next) => {
  ...
  if (req.body.parentId) {
    return next(
      new AppError("Replies are not supported for post comments", 400)
    );
  }
